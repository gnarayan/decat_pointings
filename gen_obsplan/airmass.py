...
class AirmassCalculator:
    ...
    def _get_hour_angles(self, ra, times):
        ...
        try:
            ...
            # AR: make sure the hour angle is between -180.0 and 180.0
            has = np.where(has>180.0,has-360.0,has)
            return has
        except:
            if not isinstance(times, Time):
                return np.nan
            has = self._loc.local_sidereal_time(times).to(u.deg).value - ra.to(u.deg).value
            # AR: make sure the hour angle is between -180.0 and 180.0
            has = np.where(has>180.0,has-360.0,has)
            return has
